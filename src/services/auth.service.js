...
export const registrar = async ({ nombre, email, password }) => {
  ...
  console.log( existente); // Agrega este log para verificar el usuario encontrado  
  if (existente) {
...
export const iniciarSesion = async ({ email, password }) => {
  if (!email || !password) {
    throw new AppError('Se requieren email y contraseña', 400);
  }
  const usuario = await UsuarioRepository.findByEmail(email);
  console.log(usuario); // Agrega este log para verificar el usuario encontrado 
